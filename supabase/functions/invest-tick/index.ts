...
//
// 매번 하는 일:
//  1) 오늘(가장 최근 거래일) S&P500 종가를 가져와 market_snapshots 에 저장
//  2) 마지막 이자 지급 이후 15일이 지났으면, 그 구간을 반으로 나눠 앞뒤 평균을
//     비교해 등락률을 구하고 1~7% 로 매핑한 뒤 apply_invest_tick() 호출
//
// 지수를 못 가져와도(휴장일 등으로 오늘치가 비어도) 절대 실패로 죽지 않는다 —
// 구간 평균 계산에 데이터가 모자라면 그냥 최소이율(1%)로 떨어질 뿐이다.
...
Deno.serve(async (req) => {
  try {
    ...
    await fetchAndStoreLatest(admin);

    if (forceStart && forceEnd) {
      const result = await evaluateWindow(admin, forceStart, forceEnd);
    ...
    const result = await evaluateWindow(admin, windowStart, windowEnd);
...
async function evaluateWindow(admin: ReturnType<typeof createClient>, windowStart: string, windowEnd: string) {
  const { data: rows } = await admin
    .from("market_snapshots").select("date, value")
    .gte("date", windowStart).lte("date", windowEnd).order("date", { ascending: true });

  const { changePct, ratePct } = computeRate(rows ?? []);
...
function computeRate(rows: { date: string; value: number }[]) {
  if (rows.length < 2) return { changePct: 0, ratePct: 1 };
  ...
  if (!(a > 0)) return { changePct: 0, ratePct: 1 };
  const changePct = (b / a - 1) * 100;
  const ratePct = changePct <= 0 ? 1 : changePct >= 10 ? 7 : 1 + 6 * (changePct / 10);
  return { changePct: round2(changePct), ratePct: round2(ratePct) };
}

async function fetchAndStoreLatest(admin: ReturnType<typeof createClient>) {
  const latest = await fetchStooqLatest().catch(() => null) ?? await fetchYahooLatest().catch(() => null);
...
async function fetchStooqLatest(): Promise<{ date: string; value: number; source: string }> {
...
async function fetchYahooLatest(): Promise<{ date: string; value: number; source: string }> {
  const res = await fetch("https://query1.finance.yahoo.com/v8/finance/chart/%5EGSPC?range=5d&interval=1d");
